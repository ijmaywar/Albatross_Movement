################################################################################
# Ian's GPS L1 to L2 code. Interpolates GPS data to 10 min (3600s) intervals
################################################################################
...
L2_dir <- paste0(GD_dir, "L2/",location,"/Tag_Data/GPS/",szn,"/3600s/")
...
for (i in 1:length(gpsfiles)) {
  ...
  if (is_empty(tripscheck)) {
    ...
    traj_3600s<-redisltraj(traj_m,3600,type="time") 
    ...
    # And Save
    df3600_keep$datetime <- as.character(format(df3600_keep$datetime)) # safer for writing csv in character format
    write.csv(df3600_keep, file=paste0(L2_dir, mi$id[1], "_GPS_L2_3600s.csv"), row.names=FALSE)
