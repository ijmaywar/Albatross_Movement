function filtData = lowpassFilt2(rawData, method, option)
% filtData = lowpassFilt(rawData, method, option)
%
%   Currently, the function only supports 'moveAvrg' , 'max' , and 'std'
%   methods
...
if strcmpi(method, 'max')
    ...
    % add dummy raw data as constant value of the mean of last interval.
    DELAY_IND_NUM = floor((AVRG_WINDOW-1)/2); % compensate for the delay in filtered data
    avrgLastSeg = max(rawData(end-DELAY_IND_NUM+1:end, :));
    rawDataDummy = [rawData; repmat(avrgLastSeg, DELAY_IND_NUM, 1)];
...
if strcmpi(method, 'std')
    ...
    % add dummy raw data as constant value of the mean of last interval.
    DELAY_IND_NUM = floor((AVRG_WINDOW-1)/2); % compensate for the delay in filtered data
    avrgLastSeg = std(rawData(end-DELAY_IND_NUM+1:end, :));
    rawDataDummy = [rawData; repmat(avrgLastSeg, DELAY_IND_NUM, 1)];
