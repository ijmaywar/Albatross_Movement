...
%first pattern can be occasionally wrong; addition 17.05.2020
%Let's find out how many different pattern we have and numbers of their detections
NDetections = length(HeaderPosition);
Indexes = [];   %only first index is registered
Detections = [];
NDetected = zeros(1,NDetections); %each position is detected at least once, but will be filled
for I_t = 1:NDetections
    if NDetected(I_t)==0 %novel element
      NDetected(I_t) = 1;
      Indexes = [Indexes I_t];
      Detections = [Detections 1];
    end    
    for J_t = (I_t+1):NDetections
      if NDetected(J_t)==0  % not yet processed
        if CC{I_t}==CC{J_t}
          Detections(end) = Detections(end)+1;
          NDetected(I_t) = NDetected(I_t)+1;
          NDetected(J_t) = NDetected(J_t)+1;
        end    
      end    
    end
end

[DetectionsSorted, ID] = sort(Detections,'descend');
IndexesSorted = Indexes(ID);
if DetectionsSorted(1)>1
    BestIndex = IndexesSorted(1);
else
    BestIndex = 1; %if only single detectioins, take the first one as it was before 17.05.2020
end

%here we have to remove labels that has wrong content not matching first configuration
for I_t =length(HeaderPosition):-1:1
   r = isequal(CC{BestIndex},CC{I_t});
   if (~r)
